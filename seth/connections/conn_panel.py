...
from seth.connections import count_popup, descriptor_popup, entries, conn_entry, circ_entry
from seth.util import panel, tor_controller, tracker, ui_tools
...
class ConnectionPanel(panel.Panel, threading.Thread):
  ...
  def __init__(self, stdscr):
    ...
    # while we're running (... but seth doesn't allow for that atm)

    if not CONFIG["features.connection.showIps"] and CONFIG["features.connection.listing_type"] == 0:
      seth_config = conf.get_config("seth")
      seth_config.set("features.connection.listing_type", Listing.keys()[Listing.index_of(Listing.FINGERPRINT)])
  ...
  def set_sort_order(self, ordering = None):
    ...
    if ordering:
      ...
      seth_config.set("features.connection.order", ", ".join(ordering_keys))
  ...
  def set_listing_type(self, listing_type):
    ...
    seth_config = conf.get_config("seth")
    seth_config.set("features.connection.listing_type", Listing.keys()[Listing.index_of(listing_type)])
  ...
  def show_sort_dialog(self):
    ...
    results = seth.popups.show_sort_dialog(title_label, options, old_selection, option_colors)
  ...
  def handle_key(self, key):
    with self.vals_lock:
      if key.is_scroll():
        ...
        selection = seth.popups.show_menu(title, options, old_selection)
        ...
        options.remove(seth.connections.entries.ListingType.HOSTNAME)
  ...
  def run(self):
    ...
    # run during seth's interface initialization (otherwise there's a
  ...
  def get_help(self):
    resolver_util = seth.util.tracker.get_connection_tracker().get_custom_resolver()
  ...
  def _update(self):
    ...
    if not seth.util.tracker.get_connection_tracker().is_alive():
      return
