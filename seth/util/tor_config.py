...
conf.get_config("seth").add_listener(general_conf_handler, backfill = True)
    ...
    if load_path:
      ...
      for line in man_call_results:
        ...
        if is_opt_indent or is_category_line:
          ...
          # not. This isn't necessary for seth, so if unable to make the check
...
def load_configuration_descriptions(path_prefix):
  ...
  if CONFIG["features.config.descriptions.enabled"]:
    ...
    # finally fall back to the cached descriptors provided with seth (this is
