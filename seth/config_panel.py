"""
Panel presenting the configuration state for tor or seth. Options can be edited
...
# TODO: The seth use cases are incomplete since they currently can't be
...
class ConfigEntry():
  ...
  def __init__(self, option, type, is_default):
    ...
    # Fetches extra infromation from external sources (the seth config and tor
...
class ConfigPanel(panel.Panel):
  """
  Renders a listing of the tor or seth configuration state, allowing options to
  ...
  def _load_config_options(self):
    """
    Fetches the configuration options available from tor or seth.
    """
  ...
  def show_write_dialog(self):
    ...
    try:
      ...
      while True:
        ...
        key = seth.controller.get_controller().key_input()
