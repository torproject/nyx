"""
Main interface loop for seth, periodically redrawing the screen and issuing
...
import seth.menu.menu
import seth.popups
import seth.header_panel
import seth.log_panel
import seth.config_panel
import seth.torrc_panel
import seth.graph_panel
import seth.connections.conn_panel
...
from seth.util import panel, tor_config, tor_controller, ui_tools
...
def get_controller():
  """
  Provides the seth controller instance.
  """
...
def init_controller(stdscr, start_time):
  ...
  sticky_panels = [
    seth.header_panel.HeaderPanel(stdscr, start_time),
    LabelPanel(stdscr),
  ]
  ...
  if CONFIG["features.panels.show.graph"]:
    first_page_panels.append(seth.graph_panel.GraphPanel(stdscr))

  if CONFIG["features.panels.show.log"]:
    expanded_events = seth.arguments.expand_events(CONFIG["startup.events"])
    first_page_panels.append(seth.log_panel.LogPanel(stdscr, expanded_events))
  ...
  if CONFIG["features.panels.show.connection"]:
    page_panels.append([seth.connections.conn_panel.ConnectionPanel(stdscr)])
    ...
    if controller.get_conf("DisableDebuggerAttachment", None) == "1":
      log.notice("Tor is preventing system utilities like netstat and lsof from working. This means that seth can't provide you with connection information. You can change this by adding 'DisableDebuggerAttachment 0' to your torrc and restarting tor. For more information see...\nhttps://trac.torproject.org/3313")
      seth.util.tracker.get_connection_tracker().set_paused(True)
    else:
      ...
      if tor_pid:
        ...
        resolver = seth.util.tracker.get_connection_tracker()
        ...
        seth.util.tracker.get_connection_tracker().set_paused(not controller.is_alive())

  # third page: config

  if CONFIG["features.panels.show.config"]:
    page_panels.append([seth.config_panel.ConfigPanel(stdscr, seth.config_panel.State.TOR)])

  # fourth page: torrc

  if CONFIG["features.panels.show.torrc"]:
    page_panels.append([seth.torrc_panel.TorrcPanel(stdscr, seth.torrc_panel.Config.TORRC)])
...
class Controller:
  ...
  def get_data_directory(self):
    """
    Provides the path where seth's resources are being placed. The path ends
...
def start_seth(stdscr):
  ...
  for key in conf.get_config("seth").unused_keys():
  ...
  log.info("seth started (initialization took %0.3f seconds)" % (time.time() - CONFIG['start_time']))
  ...
  while not control.is_done():
    ...
    if key.match('right'):
      ...
      seth.menu.menu.show_menu()
    elif key.match('q'):
      # provides prompt to confirm that seth should exit

      if CONFIG["features.confirmQuit"]:
        msg = "Are you sure (q again to confirm)?"
        confirmation_key = seth.popups.show_msg(msg, attr = curses.A_BOLD)
      ...
      override_key = seth.popups.show_help_popup()
    elif key == ord('l') - 96:
