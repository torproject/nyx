"""
Commandline argument parsing for seth.
"""
...
from seth.util import tor_controller, msg
...
DEFAULT_ARGS = {
  ...
  'config': os.path.expanduser("~/.seth/sethrc"),
  'debug_path': None,
  'logged_events': 'N3',
  'print_version': False,
  'print_help': False,
...
def get_version():
  ...
  return msg(
    ...
    version = seth.__version__,
    date = seth.__release_date__,
  )
...
def expand_events(flags):
  ...
  * 12345 - seth/stem runlevel and higher (ARM_DEBUG - ARM_ERR)
  ...
  seth_runlevels = ['ARM_' + runlevel for runlevel in tor_runlevels]

  for flag in flags:
    if flag == 'A':
      return set(list(TOR_EVENT_TYPES) + seth_runlevels + ['UNKNOWN'])
    elif flag == 'X':
      return set()
      ...
      if flag in 'DINWE':
        ...
        runlevels = seth_runlevels[runlevel_index:]
...
def missing_event_types():
  """
  Provides the event types the current tor connection supports but seth
