...
from seth.util import msg, tor_controller, panel, tracker
...
class HeaderPanel(panel.Panel, threading.Thread):
  ...
  def send_newnym(self):
    ...
    if not self.is_wide():
      seth.popups.show_msg('Requesting a new identity', 1)
  ...
  def handle_key(self, key):
    if key.match('n'):
      ...
      # seth. This is exposing two bugs...
      #
      # * This should be working. That's a stem issue.
      # * Our interface shouldn't be locking up. That's an seth issue.
      ...
      try:
        ...
        try:
          ...
          password = seth.popups.input_prompt('Controller Password: ')

          if password:
            controller.authenticate(password)

        log.notice("Reconnected to Tor's control port")
        seth.popups.show_msg('Tor reconnected', 1)
      except Exception as exc:
        seth.popups.show_msg('Unable to reconnect (%s)' % exc, 3)
        controller.close()
    else:
      return False
  ...
  def _draw_platform_section(self, x, y, width, vals):
    ...
    Section providing the user's hostname, platform, and version information...
      ...
      seth - odin (Linux 3.5.0-52-generic)        Tor 0.2.5.1-alpha-dev (unrecommended)
  ...
  def _draw_resource_usage(self, x, y, width, vals):
    ...
    System resource usage of the tor process...
      ...
      cpu: 0.0% tor, 1.0% seth    mem: 0 (0.0%)       pid: 16329  uptime: 12-20:42:07
    """
    ...
    sys_fields = (
      (0, vals.format('cpu: {tor_cpu}% tor, {seth_cpu}% seth')),
  ...
  def _update(self):
    ...
    if previous_height != self.get_height():
      ...
      seth.controller.get_controller().redraw()
    else:
...
def get_sampling(last_sampling = None):
  ...
  seth_total_cpu_time = sum(os.times()[:3])
  ...
  if last_sampling:
    seth_cpu_delta = seth_total_cpu_time - last_sampling.seth_total_cpu_time
    seth_time_delta = retrieved - last_sampling.retrieved

    python_cpu_time = seth_cpu_delta / seth_time_delta
    sys_call_cpu_time = 0.0  # TODO: add a wrapper around call() to get this

    seth_cpu = python_cpu_time + sys_call_cpu_time
  else:
    seth_cpu = 0.0
  ...
  attr = {
    ...
    'seth_total_cpu_time': seth_total_cpu_time,
