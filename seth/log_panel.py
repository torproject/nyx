...
import seth.popups
from seth import __version__
from seth.util import panel, tor_controller, ui_tools
...
def load_log_messages():
  ...
  seth_config = conf.get_config("seth")

  COMMON_LOG_MESSAGES = {}

  for conf_key in seth_config.keys():
    if conf_key.startswith("dedup."):
      event_type = conf_key[4:].upper()
      messages = seth_config.get(conf_key, [])
...
class LogPanel(panel.Panel, threading.Thread, logging.Handler):
  """
  Listens for and displays tor, seth, and stem events. This can prepopulate
  ...
  def __init__(self, stdscr, logged_events):
    ...
    if CONFIG["features.log_file"]:
      ...
      try:
        ...
        log.notice("seth %s opening log file (%s)" % (__version__, log_path))
  ...
  def reprepopulate_events(self):
    """
    Clears the event log and repopulates it from the seth and tor backlogs.
    """
  ...
  def set_duplicate_visability(self, is_visible):
    ...
    seth_config = conf.get_config("seth")
    seth_config.set("features.log.showDuplicateEntries", str(is_visible))
  ...
  def register_tor_event(self, event):
    ...
    if isinstance(event, events.CircuitEvent):
      ...
      color = "yellow"
    elif event.type not in seth.arguments.TOR_EVENT_TYPES.values():
  ...
  def show_filter_prompt(self):
    ...
    regex_input = seth.popups.input_prompt("Regular expression: ")
    ...
    if regex_input:
      try:
        ...
        seth.popups.show_msg("Unable to compile expression: %s" % exc, 2)
  ...
  def show_event_selection_prompt(self):
    ...
    popup, width, height = seth.popups.init(11, 80)
    ...
    if popup:
      try:
        ...
        user_input = seth.popups.input_prompt("Events to log: ")

        if user_input:
          user_input = user_input.replace(' ', '')  # strips spaces

          try:
            self.set_logged_events(seth.arguments.expand_events(user_input))
          except ValueError as exc:
            seth.popups.show_msg("Invalid flags: %s" % str(exc), 2)
      finally:
        seth.popups.finalize()
  ...
  def show_snapshot_prompt(self):
    ...
    path_input = seth.popups.input_prompt("Path to save log snapshot: ")

    if path_input:
      try:
        self.save_snapshot(path_input)
        seth.popups.show_msg("Saved: %s" % path_input, 2)
      except IOError as exc:
        seth.popups.show_msg("Unable to save snapshot: %s" % exc.strerror, 2)
  ...
  def handle_key(self, key):
    if key.is_scroll():
      ...
      key_press = seth.popups.show_msg(msg, attr = curses.A_BOLD)
      ...
      try:
        selection = seth.popups.show_menu("Log Filter:", options, old_selection)
  ...
  def set_event_listening(self, events):
    ...
    tor_events = events.intersection(set(seth.arguments.TOR_EVENT_TYPES.values()))
    seth_events = events.intersection(set(["ARM_%s" % runlevel for runlevel in log.Runlevel.keys()]))

    # adds events unrecognized by seth if we're listening to the 'UNKNOWN' type

    if "UNKNOWN" in events:
      tor_events.update(set(seth.arguments.missing_event_types()))
    ...
    return sorted(tor_events.union(seth_events))
  ...
  def _get_title(self, width):
    ...
    if not events_list:
      ...
      # does the following with all runlevel types (tor, seth, and stem):
