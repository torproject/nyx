...
class ConnectionStats(GraphCategory):
  ...
  def bandwidth_event(self, event):
    ...
    for entry in seth.util.tracker.get_connection_tracker().get_value():
...
class ResourceStats(GraphCategory):
  ...
  def bandwidth_event(self, event):
    resources = seth.util.tracker.get_resource_tracker().get_value()
...
class GraphPanel(panel.Panel):
  ...
  def handle_key(self, key):
    if key.match('r'):
      ...
      selection = seth.popups.show_menu('Update Interval:', list(Interval), list(Interval).index(self.update_interval))
