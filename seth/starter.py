...
import seth.util.panel
import seth.util.tor_config
import seth.util.tracker
import seth.util.ui_tools

import stem
import stem.util.log
import stem.util.system

from seth.util import log, BASE_DIR, init_controller, msg, uses_settings
...
def main(config):
  ...
  try:
    args = seth.arguments.parse(sys.argv[1:])
  ...
  if args.print_help:
    print seth.arguments.get_help()
    sys.exit()
  elif args.print_version:
    print seth.arguments.get_version()
    sys.exit()
  ...
  try:
    ...
    seth.util.panel.HALT_ACTIVITY = True
...
def _setup_debug_logging(args):
  ...
  sethrc_content = "[file doesn't exist]"

  if os.path.exists(args.config):
    try:
      with open(args.config) as sethrc_file:
        sethrc_content = sethrc_file.read()
    except IOError as exc:
      sethrc_content = "[unable to read file: %s]" % exc.strerror

  log.trace(
    'debug.header',
    seth_version = seth.__version__,
    stem_version = stem.__version__,
    python_version = '.'.join(map(str, sys.version_info[:3])),
    system = platform.system(),
    platform = ' '.join(platform.dist()),
    sethrc_path = args.config,
    sethrc_content = sethrc_content,
  )


@uses_settings
def _load_user_sethrc(path, config):
  """
  Loads user's personal sethrc if it's available.
  """
...
def _warn_if_root(controller):
  """
  Give a notice if tor or seth are running with root.
  """
  ...
  if tor_user == 'root':
    ...
    log.notice('setup.seth_is_running_as_root', tor_user = tor_user)
...
def _notify_of_unknown_events():
  """
  Provides a notice about any event types tor supports but seth doesn't.
  """

  missing_events = seth.arguments.missing_event_types()
...
def _load_tor_config_descriptions():
  ...
  seth.util.tor_config.load_configuration_descriptions(BASE_DIR)
...
def _use_unicode(config):
  ...
  if is_lang_unicode and seth.util.ui_tools.is_wide_characters_supported():
...
def _set_process_name():
  ...
  stem.util.system.set_process_name("seth\0%s" % "\0".join(sys.argv[1:]))
