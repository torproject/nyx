...
import seth.menu.actions

from seth.util import ui_tools
...
class MenuCursor:
  ...
  def handle_key(self, key):
    is_selection_submenu = isinstance(self._selection, seth.menu.item.Submenu)
...
def show_menu():
  popup, _, _ = seth.popups.init(1, below_static = False)
  ...
  try:
    ...
    menu = seth.menu.actions.make_menu()
...
def _draw_submenu(cursor, level, top, left):
  ...
  popup, _, _ = seth.popups.init(len(submenu.get_children()), menu_width, top, left, below_static = False)
