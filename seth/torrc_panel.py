"""
Panel displaying the torrc or sethrc with the validation done against it.
"""
...
# TODO: The sethrc use case is incomplete. There should be equivilant reloading
...
class TorrcPanel(panel.Panel):
  """
  Renders the current torrc or sethrc with syntax highlighting in a scrollable
  area.
  """
  ...
  def reload_torrc(self):
    ...
    self._last_content_height_args = None
    self.redraw(True)
    seth.popups.show_msg(result_msg, 1)
  ...
  def draw(self, width, height):
    ...
    if self.config_type == Config.TORRC:
      ...
      loaded_sethrc = conf.get_config("seth")
      conf_location = loaded_sethrc._path
      rendered_contents = list(loaded_sethrc._raw_contents)
